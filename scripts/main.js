...
class sqlExercise extends HTMLElement {
  ...
  connectedCallback() {
    ...
    form['onsubmit'] = (e) => {
      ...
      var sqlStatement = editor.getValue().trim().toUpperCase();
      if (sqlStatement.startsWith('SELECT')) {
        query(sqlStatement, handleSubmit, handleError);
              outputBox.innerHTML = '';
      outputBox.appendChild(result_div);
        // alert("Hi");
      } else {
        result_div.className = 'returnError';
        result_div.innerText = 'Only SELECT statements are allowed.';
        outputBox.innerHTML = '';
        outputBox.appendChild(result_div);
      }
    ...
    // Add the necessary attributes to the button
    button.setAttribute('class', 'button');
    button.setAttribute('href', 'suspectListQ1.html');
    // Set the button text
